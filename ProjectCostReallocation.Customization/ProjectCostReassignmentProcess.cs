...
namespace ProjectCostReallocation
{
  public class ProjectCostReassignmentProcess : PXGraph<ProjectCostReassignmentProcess>
  {
        ...
        {
            var registerGraph = CreateInstance<RegisterEntry>();
            ...
            {
                using (var calculator = new PMCostReassignmentProcessor(registerGraph, reassignmentGraph))
                {
                    try
                    {
                        calculator.ProcessReassigment(reassigmentPair);
                        registerGraph.Clear(PXClearOption.ClearAll);
                        reassignmentGraph.Clear(PXClearOption.ClearAll);
                    }
                    catch (Exception e)
                    {
